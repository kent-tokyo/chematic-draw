...
This is the evidence matrix for the `1.0.10` release line. A checked
...
| P0 | CDXML writer, matrix, round-trip corpus | Ready for supported subset | Multi-page parser/writer round-trips page metadata and supported annotations; molecule corpus and loss boundaries pass. Upstream Presentation semantics remain externally gated by chematic #446. |
| P1 | Query atoms/bonds, SMARTS, Markush/R-group/polymer | Ready for bounded subset | Versioned query editor, WASM-backed worker matching, immutable Markush/polymer operations, deterministic bounded expansion, and preserve-or-reject tests pass. General upstream semantics remain externally gated by chematic #447. |
