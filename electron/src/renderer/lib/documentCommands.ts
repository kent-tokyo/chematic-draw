...
export function validateMoleculeDocument(molecule: MoleculeDto): string[] {
  ...
  for (const atom of molecule.atoms) {
    ...
    atomIds.add(atom.id);
    if (!atom.element || !Number.isFinite(atom.x) || !Number.isFinite(atom.y)) errors.push(`Atom ${atom.id} has invalid identity or coordinates`);
  }
  ...
  for (const bond of molecule.bonds) {
    ...
    if (![1, 2, 3, 4].includes(bond.order)) errors.push(`Bond ${bond.id} has unsupported order`);
  }
  return errors;
}
